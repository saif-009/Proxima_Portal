  ...
  return (
    ...
    >
      <CardHeader className="flex flex-row items-center justify-between  pb-2 !pt-2">
        ...
        {metric.imagePath && (
          <Image src={metric.imagePath} height={48} width={48} alt='' className='-mr-2 '/>
        )}
      </CardHeader>
      <CardContent className='!pb-4'>
        <div className="md:text-2xl text-lg font-bold">
          {metric.id === 'total_spend' || metric.id === 'cost_per_lead' ? '₹' : ''}
          {metric.id === '' ? metric.value : FormatNumber(Math.round(Number(metric.value)))}
          {metric.id === 'conversion_rate' || metric.id === 'ctr' ? '%' : ''}
        </div>
        <p className={`md:text-md text-sm flex items-center text-black dark:text-white`}>
          {metric?.isIncrease === 'inc' ? 
            ...
            ''}
          <span className={
            metric?.isIncrease === 'inc' ? 'text-green-600' : 
            metric?.isIncrease === 'dec' ? 'text-red-600' : ''
          }>
            {metric?.perc ? 
              metric.id === 'ctr' ? 
      ...
      <DndProvider backend={HTML5Backend}>
        <div className='w-full h-full md:p-5 p-3 md:mb-0'>
          ...
          <div className="flex lg:flex-row h-full gap-5 lg:gap-0 flex-col-reverse lg:items-start relative lg:space-x-4 mb-8 ">
            <div className="lg:w-96 w-full flex-grow overflow-hidden  ">
              {/* {dashboardLoading ? (
          ...
          <CardContent>
            <Table className='rounded-lg'>
              ...
              <TableBody>
                ...
                <>
                    <TableRow onClick={() => router.push('/')} className=' cursor-pointer'>
                      <TableCell>
                        Refreshing Cola Blast
                      </TableCell>
                      <TableCell>
                        $5000
                      </TableCell>
