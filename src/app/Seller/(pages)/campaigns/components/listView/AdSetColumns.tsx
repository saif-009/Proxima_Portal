...
import { getFormattedDate, FormatNumber, formatPercentage, formatCurrency, formatDuration } from './utils'
...
export default function AdSetColumns() {
  return [
  {
        ...
        Cell: ({ row, value }:any) => (
          <div className="flex items-center  justify-start  space-x-2 pl-10" style={{ maxWidth: '100%', overflow: 'hidden' }}>
            <Button
              variant="ghost"
              size="sm"
      ...
      },
      {
        ...
        minWidth: 100,
        width: 100,
        Cell: ({ value }:any) => `₹${FormatNumber(Math.round(Number(value)) || 0)}`,
      },
      {
          ...
          const convRate =
            Number(value.clicks) > 0
              ? value.leads && value.clicks
                ? ((Number(value.leads) / Number(value.clicks)) * 100)?.toFixed(
                    2,
                  )
                : 0
              : 0
